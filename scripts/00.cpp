...
using ii_umap = gp_hash_table<pair<T1,T2>,T3,iihash>;
template<class T1, class T2>
using f_umap = gp_hash_table<T1,T2,chash>;
template<class T1, class T2>
using o_umap = gp_hash_table<T1,T2>;
#define sqr(x) (x)*(x)
...
template <typename T>
inline T fgcd(T a, T b) {while(b) swap(b, a %= b); return a;}
ll fpow(ll a, ll b, ll m) {ll o = 1; a %= m; for(;b;b >>= 1) {if(b & 1) o = o * a % m;a = a * a % m;} return 0;}
template<class T>
void uniq(vt<T> &a) {sort(all(a));a.resize(unique(all(a)) - a.begin());}
